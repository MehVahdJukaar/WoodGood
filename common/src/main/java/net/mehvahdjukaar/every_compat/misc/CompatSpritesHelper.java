...
//            TextureCache.registerSpecialTextureForBlock(Blocks.CACTUS"stripped_cactus_log_top", res("block/stripped_cactus_top"));
        ...
        // -------------------- Alex's Caves @ Temp disabled for now
...
//        addOptional("alexscaves", "stripped_thornwood_log", "_top", "block/stripped_thornwood_log_top");
        ...
        addOptional("extendedmushrooms:poisonous_mushroom_stem_stripped", "_top", "extendedmushrooms:block/poisonous_mushroom_stem_stripped");
    ...
    private static void strangewoodPattern(TextureImage image, ResourceManager manager) {
        try (TextureImage vineOverlay = TextureImage.open(manager,
            ...
            try (TextureImage temp = respriter.recolorWithAnimationOf(logTexture)) {
                TextureOps.applyOverlayOnExisting(image, temp, vineOverlay);
            }
        } catch (Exception e) {
            EveryCompat.LOGGER.warn("Failed to apply vine overlay for strangewood texture: {}", String.valueOf(e));
        }
    }
