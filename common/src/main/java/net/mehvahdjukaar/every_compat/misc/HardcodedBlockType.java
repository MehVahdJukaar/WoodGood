...
public class HardcodedBlockType {
    ...
    //would be better to have wood namespace and id either path and namespace or just full id. not a mix.
    private record PendingBlockInfo(String woodNamespace, String woodFullId, String modThatTheBlockIsFor, String blockName) {

        //TOOD: use these instead
        public boolean isForMod(String modId) {
            return this.modThatTheBlockIsFor.matches(modId);
        }
        ...
        public boolean checkMatch(String supportedModId, String woodTypeNamespace, String woodTypeFullId, String blockName) {
            ...
            if (!supportedModId.isEmpty() && !(modThatTheBlockIsFor.matches(supportedModId))) {
                return false;
            }

            if (!woodTypeNamespace.isEmpty() && !(woodNamespace.matches(woodTypeNamespace))) {
                return false;
            }

            if (!woodTypeFullId.isEmpty() && !(woodFullId.matches(woodTypeFullId))) {
                return false;
            }

            if (!blockName.isEmpty() && !(this.blockName.matches(blockName))) {
                return false;
            }
    ...
    @Nullable
    public static Boolean isWoodBlockAlreadyRegistered(String entrySetId, String blockName, WoodType woodType, String modThatTheBlockIsFor) {
        var woodNamespace = woodType.getNamespace();
        var woodFullId = woodType.getId().toString();

        PendingBlockInfo bi = new PendingBlockInfo(woodNamespace, woodFullId, modThatTheBlockIsFor, blockName);


        String blockId = modThatTheBlockIsFor + "/" + woodNamespace + "/" + blockName;

        /// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ SPECIAL EXCLUSION ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

        // Exclude blocks from a mod that are both Supported-Mod & Wood-Mod
        if (woodType.getNamespace().equals(modThatTheBlockIsFor)) return false;
        ...
        if (bi.checkMatch("dawnoftimebuilder", "", "minecraft:(oak|acacia|jungle|dark_oak|spruce|mangrove|cherry)", "fancy_fence"))
            return false;

        // Chipped's glass & glass_panes has no Vanilla WoodTypes except OAK
        if (bi.checkMatch("chipped", "", "minecraft:(acacia|birch|jungle|dark_oak|spruce|mangrove|cherry)", "\\w+_glass(_pane)?"))
            return false;
        ...
        if (bi.checkMatch("mcwfences|mcwstairs", "", "marvelous_menagerie:calamites", "")) return true;
        if (bi.checkMatch("quark", "", "marvelous_menagerie:calamites", "hollow_calamites_log")) return true;

        // The WoodType from Cobblemon's Legendary Monuments has a 32x32 texture
        if (bi.checkMatch("", "", "legendarymonuments:distortion", "")) return true;

        // Supported Mods that have supportedBlockId should be excluded due to FramedBlocks
        if (bi.checkMatch("", "", "", "torch") && PlatHelper.isModLoaded("framedblocks")) return true;
        ...
        if (bi.checkMatch("valhelsia_structures", "", "natures_spirit:joshua", "")) return true;
        ...
        if (bi.checkMatch("regions_unexplored", "gardens_of_the_dead|snifferent|nethers_exoticism", "", "(whistlecane|globar|jabuticaba)_branch"))
            return true;
        ...
        if (bi.checkMatch("quark", "upgrade_aquatic|autumnity|atmospheric|environmental|caverns_and_chasms", "", ""))
            return false;

        // Better Nether & Better End have stripped_bark as stripped_wood but bark from Bewitchment caused EC to skip
        if (bi.checkMatch("bewitchment", "betternether|betterend", "", "")) return false;

        // Create's windows will be skipped blc [Let's do] Blooming Nature & Meadow already has windows
        if (bi.checkMatch("", "bloomingnature|meadow", "", "window")) return false;
        ...
        if (bi.checkMatch("architects_palette", "upgrade_aquatic", "", "driftwood_boards|river_boards")) return false;

        // Similar to above, Architect's Palette - boards will be skipped due to the existing boards in Autumnity
        if (bi.checkMatch("architects_palette", "autumnity", "", "maple_boards")) return false;

        // Ensure blocks to be generated because TerraFirmaCraft has similar name of vanilla woodType (oak, acacia, so on)
        if (bi.checkMatch("", "tfc", "", "")) return false;

        //ecologics and quark azalea. tbh not sure why needed
        if (bi.checkMatch("quark", "", "ecologics:azalea", "")) return false;

        // we always register everything for these (mehvahdjukaar)
        if (bi.checkMatch("woodworks", "architects_palette", "", "")) return false;

        // Ensure the Architects-Palette's boards are generated with Abnormal mods (Upgrade Aquatic, Woodworks)
        if (bi.checkMatch("architects_palette", "upgrade_aquatic|autumnity|atmospheric|environmental|caverns_and_chasms", "", ""))
            return false;
